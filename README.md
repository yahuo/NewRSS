...
如果宿主机启用了 Surge/Clash Enhanced Mode，公网域名可能被解析到 `198.18.0.0/15` Fake IP。此时可设置 `OUTBOUND_ALLOW_FAKE_IP=true`；该开关只放行**域名解析得到的** Fake IP，直接请求 `198.18.0.0/15` 字面地址仍会被拒绝，其他私网和 metadata 规则保持不变。
...
- `OUTBOUND_ALLOW_FAKE_IP`
  是否允许域名解析到 Surge/Clash 使用的 `198.18.0.0/15` Fake IP，默认 `false`；不放行该网段的字面 IP URL
...
Surge/Clash Enhanced Mode may resolve public hostnames into the `198.18.0.0/15` fake-IP range. Set `OUTBOUND_ALLOW_FAKE_IP=true` for that environment. The exception applies only to fake IPs returned for hostnames; literal `198.18.0.0/15` URLs, other private ranges, and metadata targets remain blocked.
...
- `OUTBOUND_ALLOW_FAKE_IP`
  Allow hostname DNS results in the Surge/Clash `198.18.0.0/15` fake-IP range, default `false`; literal URLs in that range remain blocked
